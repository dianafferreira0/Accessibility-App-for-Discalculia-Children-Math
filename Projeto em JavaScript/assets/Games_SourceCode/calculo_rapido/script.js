function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;}const TIME = 30; // Initial game duration, in seconds
...
class Expression extends React.PureComponent {
  ...
  render() {
    ...
    return /*#__PURE__*/(
      React.createElement("div", { className: "expression", ref: ref => this.ref = ref }, /*#__PURE__*/
      React.createElement("div", { className: "from" }, /*#__PURE__*/
      React.createElement("div", { className: "text" }, from), /*#__PURE__*/
      React.createElement("div", { className: "box" }, "?")), /*#__PURE__*/

      React.createElement("div", { className: "to" }, /*#__PURE__*/
      React.createElement("div", { className: "text" }, to), /*#__PURE__*/
...
class Timer extends React.PureComponent {
  ...
  render() {
    ...
    return /*#__PURE__*/(
      React.createElement("div", { className: `timer ${remaining < 5 && remaining > 0 ? 'animated bounceIn red' : ''}`, ref: ref => this.ref = ref }, m > 9 ? '' : '0', m, " : ", s > 9 ? '' : '0', s));
...
class Header extends React.PureComponent {
  ...
  render() {
    ...
    return /*#__PURE__*/(
      ...
      React.createElement(Timer, { endTime: endTime, onTimerEnd: onTimerEnd }), /*#__PURE__*/
      React.createElement("div", { className: "status" }, /*#__PURE__*/
      React.createElement("div", { className: "status-item max" }, "somas até ", max), /*#__PURE__*/
      React.createElement("div", { className: "status-item rate" }, answered, "/", asked), /*#__PURE__*/
      React.createElement("div", { className: "status-item score" },
      this.format(score),
      multiplier > 1 && /*#__PURE__*/
      React.createElement("div", { className: "multiplier" }, "x", multiplier),
      change !== 0 && /*#__PURE__*/
      React.createElement("div", { ref: ref => this.change = ref, className: "change animated" }, change > 0 ? '+' : '', change))))));
...
class MultipleChoice extends React.PureComponent {
  ...
  render() {
    ...
    return /*#__PURE__*/(
      ...
      values.map((res) => /*#__PURE__*/
      React.createElement("div", { className: `choice animated ${selected === res ? correct ? 'tada positive' : 'negative wobble' : ''}`, onClick: () => onClick(res) }, res))));
...
class Badge extends React.PureComponent {
  render() {
    ...
    return /*#__PURE__*/(
      ...
      React.createElement("div", { className: "score" }, score), /*#__PURE__*/
      React.createElement("svg", { viewBox: "0 0 31.531 31.531" }, /*#__PURE__*/
      ...
      React.createElement("path", { fill: "#34495e", d: "M15.766,4.955c-4.444,0.028-8.05,3.648-8.05,8.097c0,4.447,3.606,8.068,8.05,8.096c4.444-0.027,8.05-3.648,8.05-8.096 C23.815,8.604,20.21,4.983,15.766,4.955z" }), /*#__PURE__*/
...
class Summary extends React.PureComponent {
  ...
  render() {
    ...
    return /*#__PURE__*/(
      React.createElement("div", { class: `summary ${!this.mounted ? 'hidden' : ''} animated ${show ? 'bounceInDown' : 'bounceOutUp'}` }, /*#__PURE__*/
      React.createElement("div", { className: "title" }, /*#__PURE__*/
      React.createElement("div", { className: "big" }, "Bom trabalho!"), "A tua pontuação é:"), /*#__PURE__*/


      React.createElement(Badge, { score: score }), /*#__PURE__*/
      React.createElement("div", { className: "button", onClick: onPlayAgain }, "Jogar de novo")));
...
class Game extends React.PureComponent {
  ...
  constructor(props) {
    ...
    });const _status = this.getInitialStatus();this.state = { status: _status, selected: -1, showSummary: false, endTime: Date.now() + TIME * 1000, prev: this.generateProblem(_status.max), next: this.generateProblem(_status.max) };}getInitialStatus() {return { score: 0, max: 10, asked: 0, answered: 0, multiplier: 1 };}randomNumber(max) {return Math.floor(Math.random() * max);}generateProblem(max) {const a = this.randomNumber(max);const b = this.randomNumber(max - a);return { a, b, choices: this.getChoices(a, b, max) };}increaseScore() {const { status } = this.state;let endTime = this.state.endTime;let max = status.max; // Add time and increase max every 5 correct answers
    if (status.answered % 5 === 4) {endTime += 15000;max *= 2;}this.setState({ endTime, status: { ...status, multiplier: Math.min(status.multiplier + 1, 5), score: status.score + max * status.multiplier, asked: status.asked + 1, answered: status.answered + 1, max } });}decreaseScore() {const { status } = this.state;this.setState({ status: { ...status, multiplier: 1, score: Math.max(0, status.score - Math.floor(status.max * 0.25)), // Decrease by MAX * <Correct Answer Probability>
        asked: status.asked + 1 } });}shuffle(a) {for (let i = a.length - 1; i > 0; i--) {const j = Math.floor(Math.random() * (i + 1));[a[i], a[j]] = [a[j], a[i]];}return a;}getChoices(a, b, max) {const result = a + b;const choices = [result];while (choices.length < 4) {const choice = this.randomNumber(max);if (!choices.includes(choice)) {choices.push(choice);}}return this.shuffle(choices);}render() {
    const { prev, next, status, selected, showSummary, endTime } = this.state;

    return /*#__PURE__*/(
      React.createElement("div", { className: "game-container" }, /*#__PURE__*/
      React.createElement(Header, { status: status, endTime: endTime, onTimerEnd: this.handleOnTimerEnd }), /*#__PURE__*/
      React.createElement("div", { className: "body" }, /*#__PURE__*/
      React.createElement(Expression, { from: `${prev.a} + ${prev.b} = `, to: `${next.a} + ${next.b} = `, transitioning: selected !== -1 })), /*#__PURE__*/

      React.createElement("div", { className: "footer" }, /*#__PURE__*/
      React.createElement("div", { className: "container" }, /*#__PURE__*/
      React.createElement(MultipleChoice, { values: prev.choices, selected: selected, onClick: this.handleOnClick, correct: selected === prev.a + prev.b }))), /*#__PURE__*/
